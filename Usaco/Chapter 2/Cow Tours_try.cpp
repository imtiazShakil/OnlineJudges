...
void  find_conn(void)
{
    ...
    {
//        p1=find_set(lnks[i].frm);
//        p2=find_set(lnks[i].to);
////        if( M.find( mp( p1,p2) )==M.end() && M.find( mp(p2,p1) )==M.end()  )
////        {
//            print3(p1,p2,lnks[i].dst );
//            M[mp(p1,p2)]=1;
//            M[mp(p2,p1)]=1;
...
//    freopen("cowtour.out","w+",stdout);
    ...
    find_conn();
//    /*
//    for(int t=0;t<SZ( lnks );t++)
    for(int t=0;t<SZ( valid_connc );t++)
    {
//        if(t>1500) break;
//        print3(valid_connc[t].fs,valid_connc[t].sc,lnks[t].dst);
        mat[ valid_connc[t].fs ][ valid_connc[t].sc ]='1';
        mat[ valid_connc[t].sc ][ valid_connc[t].fs ]='1';

//        mat[lnks[t].frm][lnks[t].to]='1';
//        mat[lnks[t].to][lnks[t].frm]='1';

//        dijkstra(0);
        dijkstra(valid_connc[t].fs);
//        dijkstra(lnks[t].frm);
...
//        print2("max distance ",mx_ddd);

        tot_mx_dd=min(tot_mx_dd,mx_ddd);


        mat[ valid_connc[t].fs ][ valid_connc[t].sc ]='0';
        mat[ valid_connc[t].sc ][ valid_connc[t].fs ]='0';
//        mat[lnks[t].frm][lnks[t].to]='0';
//        mat[lnks[t].to][lnks[t].frm]='0';
