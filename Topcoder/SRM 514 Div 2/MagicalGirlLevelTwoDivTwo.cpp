    ...
    while(!Q.empty())
    {
        ...
        for(i=0;i<8;i++)
        {
            for(int j=0;j<SZ(jt);j++)
            {
                ...
                p=row+v1;
                q=col+v2;
                if( M.find(mp(p,q))==M.end() && p<=30 && p>=-30 && q<=30 && q>=-30 )
                {
                    Q.push(p);
                    Q.push(q);
                    M[mp(p,q)]=1;
...
int main(int argc, char* argv[])
{
	if (argc == 1)
	{
		cout << "Testing MagicalGirlLevelTwoDivTwo (500.0 points)" << endl << endl;
		for (int i = 0; i < 4; i++)
		{
	...
	else
	{
		...
		MagicalGirlLevelTwoDivTwo _obj;
